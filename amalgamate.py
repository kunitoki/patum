 ...
 *
 * Copyright (c) {current_year} - kunitoki <kunitoki@gmail.com>
 *
...
class SourceInfo:
	...
	def WriteAlgamationFiles(self):
		...
		with open (headerPath , 'w') as headerAmalgamation:
			headerAmalgamation.write(HEADER)
			...
			headerAmalgamation.write(FOOTER)

		return headerPath

if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='Amalgamate Sources.')
	parser.add_argument('--base', action='store', default="include/")
	parser.add_argument('--output', action='store', default="dist/")
	parser.add_argument('--name', action='store', default="patum")
